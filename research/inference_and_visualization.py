        ...
        if self._counter_font is None:
            ...
            try:
                ...
                try:
                    ...
                    self._counter_font = ImageFont.truetype('arial.ttf', size=font_size)
                except (OSError, IOError):
                    # Use default font as last resort
                    self._counter_font = ImageFont.load_default()
...
def main(args):
    ...
    # First try the --ckpt argument if provided
    if args.ckpt and os.path.exists(args.ckpt):
        try:
            ...
            if 'state_dict' in checkpoint:
                ...
                print(f"Loaded PyTorch Lightning checkpoint from {args.ckpt}")
            else:
                # Direct state dict
    ...
    # If no checkpoint provided or failed, try default weight files
    if not weight_loaded:
        weight_paths = ['best_weights_PoseRAC.pth', config.get('save_ckpt_path', 'new_weights.pth')]
    ...
    if not weight_loaded:
        raise FileNotFoundError("No valid model weights found. Please provide --ckpt or ensure weight files exist.")
